...
and the complete build command, including the Pagefind search index. No deployment
has been created by this implementation task.

## Vercel

1. Import `JHodgkins/Accessibility-PlayBook` after the local work is pushed.
...
5. Review the preview with the manual checklist in `docs/QA.md` before promotion.
...
search index. Track remaining accessibility review gaps in `PLAN.md`.
